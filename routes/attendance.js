...
router.post('/admin/schedule/:id/revert-checkout', auth, requireAdmin, async (req, res) => {
  ...
  console.log('=== ADMIN HOÀN TÁC CHECKOUT ===');
  ...
  try {
    ...
    if (record.trang_thai !== 'checked_out') {
      return res.status(400).json({ 
        success: false,
        message: 'Chỉ có thể hoàn tác checkout cho ca đã hoàn thành' 
      });
    }

    // Kiểm tra xem ca có phải là lịch ảo (trực thay) không
    if (record.loai_lich === 'virtual') {
      return res.status(400).json({ 
        success: false,
        message: 'Không thể hoàn tác checkout cho ca trực thay. Vui lòng xử lý trực tiếp qua quản lý trực thay.' 
      });
    }

    // Kiểm tra xem ca có liên quan đến trực thay không (là lịch gốc)
    const [trucThayInfo] = await db.query(
      'SELECT * FROM truc_thay WHERE lich_truc_goc_id = ? AND trang_thai IN ("active", "completed")',
      [id]
    );

    if (trucThayInfo.length > 0) {
      return res.status(400).json({ 
        success: false,
        message: 'Ca này đang được trực thay. Vui lòng xử lý qua quản lý trực thay trước khi hoàn tác.' 
      });
    }
    ...
    try {
      ...
      await db.query(
        `UPDATE lich_truc 
         SET trang_thai = 'checked_in', 
             ...
             ghi_chu = CONCAT(
               COALESCE(ghi_chu, ''), 
               ' | Admin (ID: ', ?, ') hoàn tác checkout lúc ', NOW()
             )
         WHERE id = ?`,
        [admin_id, id]
      );

      // Ghi log hành động admin (nếu có bảng log)
      try {
        await db.query(
          `INSERT INTO admin_logs (admin_id, action, target_id, details, created_at)
           VALUES (?, 'revert_checkout', ?, ?, NOW())`,
          [admin_id, id, `Hoàn tác checkout cho ca làm việc ID: ${id} của nhân viên ${record.ma_nhan_vien} (${record.ten_nhan_vien})`]
        );
      } catch (logError) {
        console.error('Lỗi ghi log (không ảnh hưởng):', logError);
      }

      await db.query('COMMIT');

      console.log('✅ Hoàn tác checkout thành công cho lịch trực ID:', id);

      res.json({ 
        success: true,
        message: `Đã hoàn tác checkout cho ca của ${record.ten_nhan_vien} thành công`,
        data: {
          id: record.id,
          nhan_vien: {
            ten: record.ten_nhan_vien,
            ma: record.ma_nhan_vien
          },
          ngay: record.ngay,
          ca: record.ca,
      ...
      await db.query('ROLLBACK');
      console.error('❌ Transaction lỗi khi hoàn tác checkout:', error);
      throw error;
    }
    ...
    res.status(500).json({ 
      success: false,
      message: 'Lỗi server khi hoàn tác checkout: ' + error.message 
    });
  }
});

// ======================
// ADMIN API: LẤY DANH SÁCH ADMIN LOGS (tùy chọn)
// ======================
router.get('/admin/logs', auth, requireAdmin, async (req, res) => {
  ...
  try {
    const [rows] = await db.query(
      `SELECT 
        al.*,
        nv.ten_nhan_vien as admin_ten,
        nv.ma_nhan_vien as admin_ma
      FROM admin_logs al
      INNER JOIN nhanvien nv ON al.admin_id = nv.id
